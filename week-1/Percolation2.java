...
public class Percolation {
	...
	int site = (i - 1)*n + j - 1;  // Index of this site in UF
	    
	if (i != 1) {
	    int above = (i-2)*n + j - 1;  // Index of site above in UF
	...
	if (i != n) {
	    int below = i*n + j - 1;  // Index of site below in UF
	    if (this.isOpen(i+1, j)) {
	...
	if (j != 1) {
	    int left = (i - 1)*n + j - 2;  // Index of left site in UF
	    if (this.isOpen(i, j-1)) uf.union(site, left);
	}

	if (j != n) {
	    int right = (i - 1)*n + j;  // Index of right site in UF
