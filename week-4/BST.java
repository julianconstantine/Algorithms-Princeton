...
public class BST<Key extends Comparable<Key>, Value> {
    // Represent BST via reference to root Node
    private Node root;

    private class Node {
        private Key key;            // BST is sorted by key
        private Value val;          // Associated data
        private Node left, right;   // Left and right subtrees
        private int size;           // Number of nodes in subtree

        // Instantiate a Node (left/right pointers set to null)
    ...
    // Return the value corresponding to given key, or null if no such key
    ...
    private Node put(Node x, Key key, Value val) {
        ...
        // Increases size of tree rooted at x by 1
        // Since this is in the private/helper function, this will recursively
        // take care of maintaining the size of every node when a new node is inserted
    ...
    private Node deleteMin(Node x) {
        ...
        // Recursively call deleteMin until you hit a node with a null left
        // pointer, then replace that node by its right link
        x.left = deleteMin(x.left);

        // Update the size of each node/subtree
    ...
    // Keep going left until you hit a null left pointer
    ...
    // Keep going right until you hit a null right pointer
