...
public class Board {
    ...
    public static int i0;       // i-coordinate of 0 element
    public static int j0;       // j-coordinate of 0 element
    ...
    public Stack<Board> neighbors() {
        // Initialize a new stack to store the neighbors
        Stack<Board> stack = new Stack<Board>();

        // System.out.println("(" + i0 + ", " + j0 + ")");

        if (j0 > 0) {
            // If not at left edge, swap 0 element with element to its left
            int[][] swapLeft = move(board, i0, j0, i0, j0-1);
            Board leftBoard = new Board(swapLeft);
            ...
            stack.push(leftBoard);
        }

        if (j0 < n-1) {
            // If not at right edge, swap 0 element with element to its right
            int[][] swapRight = move(board, i0, j0, i0, j0+1);
            Board rightBoard = new Board(swapRight);
        ...
        if (i0 > 0) {
            // If not at top edge, swap 0 element with element directly above it
            int[][] swapTop = move(board, i0, j0, i0-1, j0);
            Board topBoard = new Board(swapTop);
        ...
        if (i0 < n-1) {
            // It not at bottom edge, swap 0 element with element directly below it
            int[][] swapBottom = move(board, i0, j0, i0+1, j0);
            Board bottomBoard = new Board(swapBottom);

            System.out.println("(" + i0 + ", " + j0 + ")");

            System.out.println(bottomBoard.toString());

            stack.push(bottomBoard);
        }
    ...
    private int[][] move(int[][] points, int i1, int j1, int i2, int j2) {
        int[][] copy = new int[n][n];

        // Copy over the array
        // Note: Do NOT do int[][] copy = points, this will set the two arrays
        // to have the same reference! You want to explicitly copy the values
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                copy[i][j] = points[i][j];
            }
    ...
    public static void main(String[] args) {
        ...
        // Print out index of zero element
        System.out.println("Index: (" + i0 + ", " + j0 + ")");

        // Print out all neighbors
        // Iterable<Board> neighb = initial.neighbors();
        Stack<Board> neighb = initial.neighbors();
